...
namespace MenuSystem
{
    ...
    const glm::vec3 vTextColor = glm::vec3(0.0f, 0.0f, 1.0f);
    const glm::vec3 vTextColor_disabled = glm::vec3(0.2f, 0.2f, 0.2f);
    ...
    public:
        ...
        glm::ivec2 GetSize(TextRenderer* pTextRenderer)
        {
            glm::ivec2 vNameSize = pTextRenderer->GetStringSize(sName);
            float width = vNameSize.x / vPatchSize.x + 1; // (((vNameSize.x % vPatchSize.x) != 0) ? 1 : 0);
        ...
        void Build(TextRenderer* pTextRenderer)
        {
            ...
            this->vPatchSize = glm::ivec2(pTextRenderer->Characters['H'].Size.y + 2);
            ...
            for (auto& object : items)
            {
                if (object.HasChildren())
                    ...
                    object.vPatchSize = glm::ivec2(pTextRenderer->Characters['H'].Size.y + 2);
    ...
    protected:
        ...
        glm::ivec2 vPatchSize = glm::ivec2(32.0f);
    ...
    public:
        ...
        void DrawObject(MenuObject& menu, Texture2D& pTexGFX, glm::ivec2 vScreenOffset, float scale = 1.0f)
        {
            ...
            for (vPatchPos.x = 0; vPatchPos.x < menu.vSizeInPatches.x; vPatchPos.x++)
            {
                for (vPatchPos.y = 0; vPatchPos.y < menu.vSizeInPatches.y; vPatchPos.y++)
                {
                    ...
                    pSpriteRenderer->DrawPartialSprite(pTexGFX, vSourcePatch * patch_size, glm::vec2(patch_size), vScreenLocation, glm::vec2(menu.vPatchSize) * scale);
                }
            }
        ...
        void Draw(MenuManager& manager, Texture2D& sprGFX, glm::ivec2 vScreenOffset, float scale = 1.0f)
        {
            ...
            // Draw cursor.
            pSpriteRenderer->DrawPartialSprite(sprGFX, glm::ivec2(4, 0) * patch_size, glm::ivec2(32), manager.panels.back()->vCursorPos, glm::vec2(manager.panels.back()->vPatchSize * 2) * scale);
        }
    };
}